...
public class SubscriptionDAO implements ISubscriptionDAO {
    public static final String QUERY = "SELECT * FROM SUBSCRIPTION ";
    public static final String QUERY_COUNT = "SELECT COUNT(*) AS CCOUNT FROM SUBSCRIPTION";
...
//        Query.queryUpdate("update subscription set pay='2' where pay='0' and user_id='"+userId+"'");
    ...
    public Map<String, String> getHistory(String userId) {
        ...
        try {
            ...
            statemen = DBQuery.getConnection().prepareStatement(QUERY + "WHERE USER_ID = ? AND PAY = 1");
            statemen.setString(1, userId.toString());
...
//			System.out.println("******************************");
    ...
    public Map<String, String> getSubscriptions(String userId) {
        ...
        try {
            ...
            statemen = DBQuery.getConnection().prepareStatement(QUERY + "WHERE USER_ID = ? AND PAY = 0");
            statemen.setString(1, userId.toString());
            rSet = DBQuery.getResult(statemen);
...
//         Query.queryUpdate("update subscription set pay='1' where pay='0' and user_id='"+userId+"'");
    ...
    public boolean findSomeSubscriptionforUser(String userId, String publId) {
        ...
        try {
            ...
            statemen = DBQuery.getConnection().prepareStatement(QUERY + "WHERE USER_ID = ? AND PUBL_ID= ? AND PAY = 0");
            statemen.setString(1, userId.toString());
            statemen.setString(2, publId.toString());
            rSet = DBQuery.getResult(statemen);
